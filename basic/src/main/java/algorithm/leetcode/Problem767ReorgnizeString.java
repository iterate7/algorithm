...
public class Problem767ReorgnizeString {
	 ...
	 * 成功的组合算法如下：
	 * 
	 * 1. 根据fre统计分块。相同的字符放在一块。
	 * 2. 从每个块中轮询，每次从一个块里提取一个字符；一直如此继续下去，肯定不会存在相邻的。核心思路：每次都来自不同的block；
	 * block之间就不相同，这个思路有点先聚类，然后抽取代表的意思。
	 * 3. 直到每个block都不存在。
	 ...
	 * 组合的算法2：
	 * 1. 根据fre继续分块和统计。
	 * 2. 我们依旧是轮询，但是是每次把一个块轮询完，把位置分为叉开放置的思路，也就是每次都隔着放；这样保证每个块的字符都无法连在一起。
	 * 这个思路也是轮询思想。
	 * @param S
	 * @return
	 */
	public String reorganizeString(String s)
	{
		      int[] chars = new int[26];
				...
				while(!char2fre.isEmpty())
				{
					...
					{
						int[] second = char2fre.poll();//这个值肯定是和last不同的
